...
WITH table1 AS (
    ...
    -- Xrhsimopoihoume thn stratigiki poy einai pio konta kai profanws prin (hmerologiaka) apo to ekastote paixnidi EAN uparxei.
    AND ta_home.date = (
        SELECT max(ta2.date)
    ...
    SELECT
        ...
        ROUND (SUM(CASE WHEN home_team_goal > away_team_goal THEN 1 ELSE 0 END)* 100.0 / COUNT(*),2) home_win_percentage,
        ROUND (SUM(CASE WHEN home_team_goal < away_team_goal THEN 1 ELSE 0 END)*100.0 / COUNT(*),2) away_win_percentage
    FROM table1
    GROUP BY table1.home_buildUpPlaySpeed,
            table1.home_chancecreationshoot,
            table1.away_buildUpPlaySpeed,
            table1.away_chanceCreationShoot
    -- Theloume perissotera apo 10 paixnidia, wste na vgaloume asfali sumperasmata.
    HAVING COUNT(*) > 10
)
...
These strategies lead to a higher win percentage for the home team (69.23%), than any other combination
*/
...
-- We can continue in the same way to draw meaningful conclusions.